...
import DebenturesAccount from "../accounts/debenturesAccount";

const DebenturesComponent = ({ debenture }) => {
  const onClick = () => {
    console.log("hi");
  };
  const debentures: Debentures[] = debenture;
  console.log(debentures);
  const investedValue = debentures?.reduce(
    (accumulator, current) =>
      accumulator + Number(current.account.summary.investmentValue),
    0
  );
  const currentValue = debentures?.reduce(
    (accumulator, current) =>
          ...
          {debentures?.map((aif: Debentures) => {
            return (
              <Box
                ...
                w="100%"
              >
                <DebenturesAccount accounts={aif} />
              </Box>
            );
          })}
        </Box>
          ...
          <Table size="sm" colorScheme="blue">
            ...
            <Tbody>
              <Tr>
                ...
                <Td isNumeric>{debentures?.length}</Td>
              </Tr>
              <Tr>
...
export default DebenturesComponent;
